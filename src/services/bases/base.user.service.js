...
export class BaseUserService {
  ...
  static getInstance() {
    if (!this.instance) {
      this.instance = new BaseUserService();
    }
  ...
  */
  ...
  register = async (data) => {
    if ( await this.dbModel.findOne({ email: data.email }) ) throwBadRequestError("Email already exists");
  ...
  login = async (data) => {
    ...
    let user = await this.dbModel.findOne({ email }) ?? throwNotFoundError(`${this.modelName[0].toUpperCase() + this.modelName.slice(1)} not found`);
    const authId = generateAuthId();
    const token = this.jwtService.generateToken(authId);
